...
const { getCurrentServerConfig, printConfig, BACKEND_SERVER_URL } = serverCfg;
const { createProxyMiddleware } = require('http-proxy-middleware');

const currentConfig = getCurrentServerConfig();
// 支持环境变量 PORT，用于云平台部署
const port = process.env.PORT || currentConfig.port || 8082;
...
app.post('/api/v1/admin/streams', (req, res) => {
	...
	console.log('✅ 新增直播流 (v1):', newStream.name, newStream.url);
...
app.get('/api/v1/admin/dashboard', (req, res) => {
	try {
		...
		const data = {
			...
			isLive: globalLiveStatus.isLive,
			liveStreamUrl: globalLiveStatus.streamUrl,
			streamId: globalLiveStatus.streamId || null,
			activeStreamUrl: activeStream ? activeStream.url : null,
			activeStreamId: activeStream ? activeStream.id : null,
			activeStreamName: activeStream ? activeStream.name : null,
			totalVotes: totalVotes,
...
app.post('/api/v1/admin/live/start', (req, res) => {
	try {
		...
		broadcast('live_started', {
			data: {
				streamId: streamId,
				streamUrl: stream.url,
				streamName: stream.name,
				startTime: globalLiveStatus.startTime
			}
		});
		
		console.log('🎬 直播已开始:', stream.name, stream.url);
		
		res.json({
			success: true,
			data: globalLiveStatus,
			message: '直播已开始',
			timestamp: Date.now()
		});
...
app.post('/api/v1/admin/live/stop', (req, res) => {
	try {
		...
		const stoppedStream = { ...globalLiveStatus };
		...
		broadcast('live_stopped', {
			streamId: stoppedStream.streamId,
			stoppedAt: new Date().toISOString()
		});
		
		console.log('🛑 直播已停止:', stoppedStream.streamName);
	...
	try {
		...
		wsClients.forEach((client, ws) => {
			if (client && client.userInfo) {
				viewers.push({
					id: client.userInfo.id || client.id,
					nickname: client.userInfo.nickname || '匿名用户',
					avatar: client.userInfo.avatar || '',
					joinTime: client.joinTime || new Date().toISOString(),
					isOnline: true
				});
			}
		});
	...
	try {
		...
		res.json({
			...
			data: {
				...
				debate: debate,
				votes: currentVotes,
				isLive: globalLiveStatus.streamId === streamId && globalLiveStatus.isLive
			},
			timestamp: Date.now()
		});
		
	} catch (error) {
		console.error('获取直播流辩论信息失败:', error);
		res.status(500).json({
			success: false,
			message: '获取直播流辩论信息失败: ' + error.message
		});
	}
});

// v1 版本 - 辩论流程管理
app.get('/api/v1/admin/debate-flow', (req, res) => {
	res.json({
		success: true,
		data: {
			currentPhase: 'waiting',
			phases: ['waiting', 'opening', 'debate', 'voting', 'conclusion'],
			debate: db.debate.get()
		},
		timestamp: Date.now()
	});
});

app.post('/api/v1/admin/debate-flow', (req, res) => {
	try {
		const { phase, action } = req.body;
		
		console.log('📋 辩论流程操作:', action, '阶段:', phase);
		
		broadcast('debate_flow_update', { phase, action, timestamp: Date.now() });
		...
		console.error('辩论流程操作失败:', error);
		res.status(500).json({
			success: false,
			message: '辩论流程操作失败: ' + error.message
		});
	}
});
...
// 获取评委列表
app.get('/api/v1/admin/judges', (req, res) => {
	try {
		...
		const judges = db.judges.getByStream(stream_id);
		...
		res.status(500).json({
			success: false,
			message: '获取评委列表失败: ' + error.message
		});
	}
});

// 保存评委列表
app.post('/api/v1/admin/judges', (req, res) => {
	try {
		...
		if (!Array.isArray(judges)) {
			return res.status(400).json({
				success: false,
				message: 'judges 必须是数组'
			});
		}
		
		const savedJudges = db.judges.save(stream_id, judges);
		
		// 广播评委更新
		broadcast('judges_updated', { streamId: stream_id, judges: savedJudges });
		
		console.log('💾 保存评委数据:', stream_id, savedJudges.length, '位评委');
		...
		res.status(500).json({
			success: false,
			message: '保存评委数据失败: ' + error.message
		});
	}
});
...
app.get('/api/admin/debate-flow', (req, res) => {
	try {
		...
		const flowConfig = db.debateFlow.getByStream(stream_id);
		...
		res.status(500).json({
			success: false,
			message: '获取辩论流程配置失败: ' + error.message
		});
	}
});
...
app.post('/api/admin/debate-flow', (req, res) => {
	try {
		const { stream_id, segments, phase, action } = req.body;
		
		// 兼容两种请求格式
		const actualStreamId = stream_id;
		...
		if (actualSegments && Array.isArray(actualSegments)) {
			// 保存流程配置
			const savedFlow = db.debateFlow.save(actualStreamId, actualSegments);
			
			// 广播流程更新
			broadcast('debate_flow_updated', { streamId: actualStreamId, flow: savedFlow });
			
			console.log('💾 保存辩论流程配置:', actualStreamId, actualSegments.length, '个环节');
			...
			// 流程控制命令
			console.log('📋 辩论流程控制:', action, '阶段:', phase, '流:', actualStreamId);
			
			broadcast('debate_flow_control', { 
		...
		res.status(500).json({
			success: false,
			message: '保存辩论流程配置失败: ' + error.message
		});
	}
});
...
app.delete('/api/admin/ai/content/:contentId', (req, res) => {
	try {
		const { contentId } = req.params;
		const { reason, notifyUsers } = req.body;
		
		const deleted = db.aiContent.delete(contentId);
		
		if (deleted) {
			console.log('🗑️ 删除AI内容:', contentId, '原因:', reason);
			
			if (notifyUsers) {
				broadcast('ai_content_deleted', { contentId, reason });
			}
...
// 获取AI内容评论
app.get('/api/v1/admin/ai-content/:contentId/comments', (req, res) => {
	try {
		const { contentId } = req.params;
		const { page = 1, pageSize = 20 } = req.query;
		
		const content = db.aiContent.getById(contentId);
		...
		const items = comments.slice(startIndex, startIndex + parseInt(pageSize));
		...
		console.error('获取AI内容评论失败:', error);
		res.status(500).json({
			success: false,
			message: '获取AI内容评论失败: ' + error.message
		});
	}
});

// 删除AI内容评论
app.delete('/api/v1/admin/ai-content/:contentId/comments/:commentId', (req, res) => {
	try {
		const { contentId, commentId } = req.params;
		const { reason, notifyUsers } = req.body;
		
		const deleted = db.aiContent.deleteComment(contentId, commentId);
		
		if (deleted) {
			console.log('🗑️ 删除AI内容评论:', contentId, commentId, '原因:', reason);
			
			if (notifyUsers) {
				broadcast('ai_comment_deleted', { contentId, commentId, reason });
			}
		...
		console.error('删除AI内容评论失败:', error);
		res.status(500).json({
			success: false,
			message: '删除AI内容评论失败: ' + error.message
		});
	}
});
...
if (BACKEND_SERVER_URL) {
    app.use('/api', (req, res, next) => {
        ...
        if (req.path.startsWith('/admin') || 
            ...
            req.path.includes('/admin/streams') ||
            req.path.includes('/admin/debate') ||
            req.path.includes('/admin/users') ||
            req.path.includes('/admin/votes') ||
            req.path.includes('/admin/ai-content') ||
            req.path.includes('/admin/ai/content') ||
            req.path.includes('/admin/live') ||
            req.path.includes('/admin/dashboard') ||
            req.path.includes('/admin/statistics') ||
            req.path.includes('/admin/miniprogram') ||
            req.path.includes('/admin/debate-flow') ||
            req.path.includes('/admin/judges') ||
            req.path.includes('/v1/admin/debate-flow') ||
            req.path.includes('/v1/admin/ai-content') ||
            req.path.includes('/v1/admin/judges') ||
            req.path === '/votes' ||
            req.path === '/debate-topic' ||
            req.path === '/ai-content' ||
            req.path === '/comment' ||
            req.path === '/like' ||
            req.path === '/user-vote' ||
            req.path === '/wechat-login') {
            console.log(`📍 本地路由: ${req.method} ${req.path}`);
            return next();
        }
        
        // 其他 /api 请求代理到后端
        console.log(`🔄 代理请求: ${req.method} ${req.path} -> ${BACKEND_SERVER_URL}`);
